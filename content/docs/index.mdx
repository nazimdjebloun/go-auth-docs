...
**Remote auth** — `middleware.RemoteAuth` lets a second Go service (an admin console, a BFF, a gateway) gate routes on the same sessions by asking your go-auth server, without a database connection, a copy of your config, or your application secret.
