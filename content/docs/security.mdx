...
Where your frontend is hosted decides how much of this you get for free. Same origin needs no configuration, sibling subdomains need the CSRF cookie scoped, and a different registrable domain needs `SameSite=None` plus `ExposeCSRFTokenInBody`. See the [Deployment guide](./guides/deployment.mdx) for all three and what each one costs.

**The address recorded on a session or an audit event follows the same rules.** `middleware.ClientIP` reads `IPAddressHeader` only when the immediate peer is in `TrustedIPs`, takes the rightmost hop that isn't itself a trusted proxy, and falls back to the connection address when the header is absent, untrusted, or unparseable — so a client can never write its own audit trail. It differs from the rate-limit key in one way: IPv6 is not masked to a subnet, because this is a record of who connected rather than a bucket to group them into.
...
`WithSecret` supplies the one application-wide secret the library needs, and must be at least 32 bytes (`NewConfig` rejects anything shorter, since HMAC-SHA256 wants a full-strength key). It is never used directly as key material: it's run through HKDF-SHA256 once, with a distinct info string per purpose, to derive three independent subkeys — one for CSRF tokens, one for the 2FA challenge-binding cookie (which ties a 2FA challenge to the browser that started it, so a leaked `challengeId` alone isn't a usable credential — see [Two-factor authentication](#two-factor-authentication)), and one that encrypts OAuth provider tokens at rest with AES-256-GCM (a random nonce per encryption) — with room for more as the library adds HMAC-based tokens. A compromise of one derived purpose's key doesn't hand over the others. Nothing about the secret or its derived keys is logged or exposed via any endpoint. Rotating it invalidates every outstanding CSRF token and 2FA challenge binding immediately, and makes previously-stored OAuth provider tokens undecryptable; there's no dual-key rotation window today, so plan a rotation for a moment where forcing every open tab to re-fetch a CSRF token, retrying any in-flight 2FA challenge, and re-linking OAuth accounts is acceptable.
