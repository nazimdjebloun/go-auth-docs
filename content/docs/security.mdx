...
- **Remote services** — a separate process gating routes with `middleware.RemoteAuth` gets the same checks, enforced by the upstream server rather than duplicated locally. It holds no database credentials and no application secret, and it re-asks upstream on every request rather than caching, so a revoked session or a demoted admin stops working on the next call there too. When it *can't* reach upstream it fails closed with `503`, not `401` — undecided is not the same as unauthenticated. See [Remote Auth](guides/remote-auth.mdx).
