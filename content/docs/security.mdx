...
**Admin login always requires a second factor, unconditionally.** `POST /auth/admin/login` gates on this independent of `RequireEmail2FA`, independent of the admin account's own `TwoFactorEnabled`, with no config flag to turn it off — it applies in every deployment built via `goauth.New()`, since the 2FA service is always constructed. Concretely: a mailer becomes a hard requirement for admin login to function at all, even in a deployment that otherwise never touches any 2FA setting. See [Admin → Admin login](guides/admin.mdx#admin-login) for the request/response shape this produces.
