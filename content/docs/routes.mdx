...
|---|---|---|---|
| `POST /auth/register`, `POST /auth/signup` | Public | `` `{email, password, name}` `` | Register with email/password. Returns a session unless verification is required, or `RequireEmail2FA` is on (see [Two-factor authentication](#two-factor-authentication)). |
| `POST /auth/login`, `POST /auth/signin` | Public | `` `{email, password}` `` | Log in. May return `requiresVerification: true` instead of a session, or `requiresTwoFactor: true` if the account has 2FA on. |
| `POST /auth/admin/login` | Public | `` `{email, password}` `` | Same as login, but only succeeds for `role: admin` users. Always requires a second factor — see [Two-factor authentication](#two-factor-authentication). |
