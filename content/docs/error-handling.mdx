...
| `auth_unavailable` | 503 | Authentication service unavailable | Only from `middleware.RemoteAuth` — the upstream go-auth server couldn't be reached, so authentication is *undecided*, not failed. Distinct from `401` on purpose: retry it, don't log the user out. See [Remote Auth](guides/remote-auth.mdx). |
