...
See [Routes](../routes/oauth.mdx) for the full parameter reference, [Error Handling](../error-handling.mdx#oauth) for every error this can redirect back with, and [Guides → OAuth linking](../guides/oauth.mdx) for connecting GitHub to an account that's already logged in (a different flow from the sign-in one above).
