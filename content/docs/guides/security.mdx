...
`SessionConfig.TokenTTL` is validated with the session section — see [Sessions](./sessions.mdx): `session token_ttl must be positive`, only reachable if it was explicitly set negative.
...
See [Client → Change password](./client/security.mdx#change-password) for the wrapper and error handling.
...
See [Client → Set password](./client/security.mdx#set-password-oauth-only-accounts) for the wrapper and two-step flow.
...
See [Client → Forgot / reset password](./client/security.mdx#forgot-password) for the wrapper and two-step flow.
...
| `reset_token_expired` | 410 | Past `SessionConfig.TokenTTL` (default 1h) |
| `weak_password` | 400 | Fails `PasswordPolicy` |
...
See [Client → Forgot / reset password](./client/security.mdx#reset-password) for the wrapper and two-step flow.
...
See [Client → Resend verification](./client/security.mdx#resend-verification) for the wrapper and both logged-in and public variants.
