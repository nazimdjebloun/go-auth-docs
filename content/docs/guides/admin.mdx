...
**Response**: `{ "sessions": [...], "total": 5, "limit": 20, "offset": 0 }`
...
```ts
const { sessions, total } = await apiRequest(API_BASE, "GET", "/admin/sessions?ip=203.0.113.9");
```
...
Each field is its own `List`/`ListAll` call with `Limit: 1` under the hood — `List` always runs a `COUNT(*)` query regardless of `Limit`, so the count is exact and the row fetch itself stays trivial. `neverLoggedInUsers` is `last_login_at IS NULL` — the same filter as [List users → `neverLoggedIn`](#list-users--get-adminusers) above, just counted rather than listed.
...
**Response**: `{ "orgs": [...], "total": 42, "limit": 20, "offset": 0 }`
...
```ts
const { orgs, total } = await apiRequest(API_BASE, "GET", "/admin/orgs?search=acme");
```
...
Same query params as the self-service `GET /auth/orgs/{orgID}/members` (see [Organizations](organizations.mdx)), but bypasses the membership check that endpoint applies — the caller doesn't need to be a member of `orgID`. Publishes `admin.org.viewed`.
...
```ts
const { members, total } = await apiRequest(API_BASE, "GET", "/admin/orgs/org-123/members");
```
