...
Because [admin login always requires a second factor](#admin-login), a seeded admin who can't receive email can never log in — so this command sends a **real** verification email to the address you're seeding, before writing anything to the database, and aborts with nothing written if that send fails. `--driver`, `--dsn`, and `--env` are required; `--env` is `dev`, `staging`, or `prod` and controls which mailer default applies.
...
<Callout type="warn" title="Admin login always requires a second factor — unconditionally, not just under RequireEmail2FA">
  Every real `goauth.New()` deployment constructs the 2FA service internally, with no config flag that skips it, and `AdminLogin` gates on that unconditionally — independent of `SecurityConfig.RequireEmail2FA`, and independent of whether the admin account itself has `TwoFactorEnabled` set. In other words: this isn't "on by default and configurable" — there is currently no supported way to turn it off. This holds even in a deployment that never touches any 2FA setting at all, which has one concrete consequence worth knowing up front: **a mailer (`WithMailer`/`WithEmail`) is mandatory for `/auth/admin/login` to work at all**, since the second-factor code has to be emailed. `NewConfig`'s validation reflects this directly — a mailer is required unconditionally in every environment except `EnvironmentDev` (which defaults to a log-only mailer automatically if none is configured), regardless of `RequireEmailVerification`, `EnableInvite`, `RequireEmail2FA`, or `DefaultTwoFactorEnabled`. See [Configuration → WithMailer / WithEmail / WithTemplates](../configuration.mdx#withmailer--withemail--withtemplates--required), [Two-factor authentication](authentication.mdx#two-factor-authentication) for the verify step, and [Security](../security.mdx#two-factor-authentication) for the reasoning.
</Callout>
...
user, session, rawToken, refreshToken, err := auth.Services.TwoFactor.Verify(
