...
`total` is the count of orgs matching `search` (ignoring `limit`/`offset`), so it's what you paginate against, not `orgs.length`.
...
// result.Orgs, result.Total, result.Limit, result.Offset
```
...
```ts
const { orgs, total } = await apiRequest(
...
// result.Members, result.Total, result.Limit, result.Offset
```
...
// result.Invites, result.Total, result.Limit, result.Offset
```
...
```ts
const { invites, total } = await apiRequest(
