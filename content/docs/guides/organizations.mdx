...
```go
org, err := auth.Services.Org.GetByID(ctx, orgID)
```
...
```go
err := auth.Services.Org.DeleteOrg(ctx, orgID)
```
...
```go
members, total, err := auth.Services.Org.ListMembers(ctx, orgID, 0, 20)
```
...
err = auth.Services.Org.LeaveOrg(ctx, orgID, callerUserID)
```
...
**From a browser client**, there's no dedicated "what's my active org" endpoint — `/auth/me` returns the user, not the session. The active org is visible on the [session-listing](sessions.mdx#listing-sessions) responses (match `current_session_id` against the `sessions` array to find your own), but the simplest approach is usually to just track it as local state the moment you call `setActive`/`clearActive` below, rather than re-fetching to confirm it.
...
```go
err := auth.Services.Org.SetActiveOrg(ctx, sessionID, userID, orgID)
```
...
```go
err := auth.Services.Org.ClearActiveOrg(ctx, sessionID, orgID)
```
...
- [Sessions](sessions.mdx#listing-sessions) — where `active_org_id`/`active_org_role` show up on a session
