...
See [Create organization](./client/organizations.mdx#create-organization) for the wrapper and form component.
...
See [List organizations](./client/organizations.mdx#list-organizations) for the wrapper and data table.
...
See [Get organization](./client/organizations.mdx#get-organization) for the wrapper.
...
See [Update organization](./client/organizations.mdx#update-organization) for the wrapper and form component.
...
See [Delete organization](./client/organizations.mdx#delete-organization) for the wrapper and confirmation dialog.
...
See [List members](./client/organizations.mdx#list-members) for the wrapper and data table.
...
See [Remove member](./client/organizations.mdx#remove-member) and [Leave organization](./client/organizations.mdx#leave-organization) for the wrapper and confirmation dialogs.
...
See [Change member role](./client/organizations.mdx#change-member-role) for the wrapper and role select component.

## Active org

A session can be scoped to one org at a time — stored as `sessions.active_org_id`/`active_org_role` (see [Schemas → sessions](../schemas.mdx#sessions)), returned on every session-listing response from [Sessions](./sessions.mdx#listing-sessions). Your app reads it to decide which org's data the current request should see.
...
<Callout type="warning" title="Org authorization does not rewrite your SQL">
...
See [Set active organization](./client/organizations.mdx#set-active-organization) for the wrapper and org switcher component.
...
The client has no dedicated clear-active wrapper — call the endpoint directly (see [Setup](./client/index.mdx)).
...
See [Create invite](./client/organizations.mdx#create-invite) for the wrapper and invite form component.
...
See [Accept invite](./client/organizations.mdx#accept-invite) for the wrapper and accept form component.
...
See [List invites](./client/organizations.mdx#list-invites) for the wrapper and data table.
...
Delete here is a hard delete of the invite record — distinct from revoking a self-service signup invite (see [Routes → Invites](../routes/auth.mdx#get-invite-info)), which is a different feature with its own soft-cancel semantics.
...
See [Resend invite](./client/organizations.mdx#resend-invite) and [Delete invite](./client/organizations.mdx#delete-invite) for the wrapper and confirmation dialogs.

## Next

- [Routes](../routes/orgs.mdx) — the full path/param table for every route on this page
