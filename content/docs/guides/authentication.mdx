...
For the full path/param reference across every route (not just these three), see [Routes](../routes/). For the error envelope shape and codes shared across the whole API, see [Error Handling](../error-handling.mdx).
...
**Response — `TwoFactorConfig.RequireEmail2FA: true`** (`201 Created`) — a session is not issued; register gates on this flag alone, independent of `RequireEmailVerification`, so the two never stack (see [Two-factor authentication](#two-factor-authentication)):
...
For a complete register form with error handling, see [Client → Register](./client/authentication.mdx#register).
...
<Callout type="warn" title="A mailer is required either way">
  Email verification requires `WithMailer` or `WithEmail`. Admin login also requires a mailer while its default 2FA challenge is enabled. In `EnvironmentDev`, a required but unconfigured mailer defaults to log-only delivery. See [Configuration](../configuration.mdx#withmailer--withemail--withtemplates-required-unless-every-email-feature-is-off).
</Callout>
...
For a complete email verification form with resend, see [Client → Email verification](./client/authentication.mdx#email-verification).
...
For a complete invite registration form, see [Client → Invite registration](./client/authentication.mdx#invite-registration).
...
`invalid_credentials` for an email that doesn't exist at all runs comparisons against a startup-generated dummy hash in the currently configured hasher's format, so that request burns the same KDF work as a wrong-password one — see [Security](../security.mdx) for why that matters.
...
For a complete login form with branching logic and error handling, see [Client → Login](./client/authentication.mdx#login).

<Callout type="warn" title="Admin login has its own flow">
  `POST /auth/admin/login` is documented in [Admin](./admin.mdx#admin-login). It requires a second factor by default, independent of other 2FA settings.
</Callout>
...
| `two_factor_code_expired` | 410 | Past `TwoFactorConfig.CodeTTL` (default 5m) |
...
For a complete two-factor form with resend, see [Client → Two-factor](./client/authentication.mdx#two-factor-authentication).
...
For enabling and disabling 2FA, see [User management → Two-factor](./user-management.mdx#two-factor-authentication).
...
If you only have the session ID instead (e.g. an admin ending one specific session — see [Routes → Sessions](../routes/sessions.mdx)), use `auth.Services.Auth.Logout(ctx, sessionID)` instead — same effect, looked up differently.
...
For a complete logout button with error handling, see [Client → Logout](./client/authentication.mdx#logout).

For account deletion and name changes, see [User management](./user-management.mdx).

## Next

- [User management](./user-management.mdx) — name changes, 2FA toggle, and account deletion
- [Security](./security.mdx) — password changes and verification resend
- [Admin](./admin.mdx) — admin login and platform-wide user management
- [Routes](../routes/auth.mdx) — every other endpoint, including sessions, password, and account management
