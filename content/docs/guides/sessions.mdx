...
|---|---|---|---|---|
| `TTL` | `time.Duration` | Optional | `30d` | Absolute hard expiry — the `expiresAt` on every session in the [list responses](#listing-sessions) above. |
...
| `TokenTTL` | `time.Duration` | Optional | `1h` | Lifetime of verification and password-reset tokens. |
| `MaxLifetime` | `time.Duration` | Optional | `0` — no limit | If set, must be `>= TTL`. |
| `GraceWindow` | `*time.Duration` | Optional | `5s` | Window a just-rotated refresh token is still accepted, so two requests racing to refresh don't log the user out. `nil` (leave the field out) means "use the default"; set it with `goauth.Duration(...)` — `goauth.Duration(0)` turns it off. |
| `TouchDebounce` | `*time.Duration` | Optional | `5m` | Minimum interval between `last_active_at` writes — what makes `lastActiveAt` in the list responses a few minutes stale rather than exact. Same `nil`-vs-pointer rule as `GraceWindow`; `goauth.Duration(0)` writes on every authenticated request. |
...
It is what lets a client read the **active org** — `activeOrgId`/`activeOrgRole` — which is otherwise not exposed anywhere: `PUT`/`DELETE /auth/orgs/active` only return a bare `{"message"}`.

Which fields you actually get:

| Field | When it appears |
|---|---|
| `id`, `userId`, `isRevoked`, `expiresAt`, `createdAt` | Always. `isRevoked` is always `false` here — a revoked session never gets past `AuthMiddleware` to reach this handler. |
| `ipAddress`, `userAgent`, `parsedUA` | When they were recorded at login. `parsedUA` is derived from `userAgent`, so the two travel together. |
| `refreshExpiresAt` | Whenever a refresh cookie was issued — in practice, always for a browser login. |
| `refreshRotatedAt` | Only after the refresh token has rotated at least once. |
| `lastActiveAt` | Once the session has been touched (subject to `TouchDebounce`). |
| `activeOrgId`, `activeOrgRole` | Only while an org is active. Both absent otherwise — not `null`. |
| `revokedAt` | Never on this endpoint; see `isRevoked` above. |

<Callout type="info" title="One session shape, everywhere">
...
| Code | Status | Cause |
|---|---|---|
| `invalid_refresh` | 401 | No refresh cookie, or one that expired / was revoked |
...
For explicit refresh usage, see [Client → Refresh tokens](./client/sessions.mdx#refresh-tokens).
...
For a complete session list UI with pagination, see [Client → Sessions](./client/sessions.mdx#list-sessions-paginated).
...
For a complete session list UI, see [Client → Sessions](./client/sessions.mdx#list-all-sessions).
...
For a complete revoke button, see [Client → Sessions](./client/sessions.mdx#revoke-one-session).
...
| Code | Status | Cause |
|---|---|---|
| `invalid_input` | 400 | `sessionIds` empty, or more than 100 entries |
...
For a complete session list with revoke, see [Client → Sessions](./client/sessions.mdx#revoke-multiple-sessions).
...
For a complete "log out everywhere else" button, see [Client → Sessions](./client/sessions.mdx#revoke-all-except-current).

## Next

- [Authentication](./authentication.mdx) — register/login/logout, and the `Origin`-checking rules these endpoints share
- [Routes](../routes/sessions.mdx) — the full path/param reference, including [admin session management](../routes/admin.mdx#user-sessions)
- [Security](../security.mdx) — session hashing, refresh rotation, and the reuse-detection grace window
- [Deployment](./deployment.mdx) for how these cookies behave when the frontend and the API are on different hosts
